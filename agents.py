...
#             agent_wallet = Wallet.fetch(key)
#             agent_wallet.load_seed(file_path)
#             # Load the saved seed
#             print("Agent wallet address:",agent_wallet.default_address.address_id)
# else:
...
def swap_assets(amount: Union[int, float, Decimal], from_asset_id: str, to_asset_id: str):
    ...
    """
    assert agent_wallet is not None
    # assert amount is Number
    if agent_wallet.network_id != "base-mainnet":
        return "Error: Asset swaps are only available on Base Mainnet. Current network is not Base Mainnet."

    try:
        trade = agent_wallet.trade(cast(Union[Number, Decimal, str],amount), from_asset_id, to_asset_id)
        trade.wait()
